// Updated functions/api/upload-photos.js
// ─── CHANGE: Added API key auth so the Android app can bypass the Origin check.
// ─── Set the UPLOAD_API_KEY secret in your Cloudflare Pages dashboard to match
// ─── the key in app/build.gradle.kts (currently "dod-android-upload-2026").
//
// To deploy: copy this file to functions/api/upload-photos.js and push to GitHub.
// Then in Cloudflare dashboard → Pages → Settings → Environment variables, add:
//   UPLOAD_API_KEY = dod-android-upload-2026
...
/** Returns true if the request is authorized (valid Origin OR valid API key). */
function isAuthorized(request, env) {
  const origin = request.headers.get('Origin') || '';
  if (ALLOWED_ORIGINS.includes(origin)) return true;

  // Android app sends X-API-Key header
  const apiKey = request.headers.get('X-API-Key') || '';
  if (apiKey && env.UPLOAD_API_KEY && apiKey === env.UPLOAD_API_KEY) return true;
...
export async function onRequestPost(context) {
  ...
  try {
    // Auth check: valid Origin OR valid API key
